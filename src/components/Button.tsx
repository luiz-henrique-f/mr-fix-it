...
interface ButtonProps extends ComponentPropsWithoutRef<"button"> {
  variant?: "primary" | "outlined" | "login" | "icon";
}

function Button({ className, variant = "primary", ...props }: ButtonProps) {
  const variantClasses = {
    primary: "text-white py-2 px-4 text-sm items-center justify-center gap-2 uppercase bg-primary hover:bg-primaryDarker",
    outlined: "text-black py-2 px-4 text-sm dark:text-white items-center justify-center gap-2 uppercase bg-transparent border-2 border-solid border-black/50 dark:border-white/70 hover:bg-primary hover:border-primary dark:hover:border-primary hover:text-white",
